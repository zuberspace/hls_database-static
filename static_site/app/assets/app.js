...
// The data points plot/structure_plot at .webp files and keeps the original path
// in *_original (see tools/optimize_images.js). <picture> serves WebP where it is
// supported and falls back to the original PNG/JPG otherwise.
function picture(rel, original, cls, alt) {
  if (!rel) return "";
  const uri = (r) => `./mediafiles/${escapeHtml(r)}`;
  const source = /\.webp$/i.test(rel) ? `<source srcset="${uri(rel)}" type="image/webp">` : "";
  return `<picture>${source}<img class="${cls}" loading="lazy" decoding="async" src="${uri(original || rel)}" alt="${escapeHtml(alt)}"></picture>`;
}
...
async function renderLayerType(slug, superseded = () => false) {
  ...
  const plot = lt.plot
    ? picture(lt.plot, lt.plot_original, "object-scale-down max-w-full mx-auto", `${lt.name} layer`)
    : noData();
